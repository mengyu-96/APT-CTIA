...
def render_home():
    ...
    try:
        ...
        # Datasets
        try:
            r = requests.get(f"{BACKEND_URL}/api/datasets", timeout=1)
            if r.status_code == 200: n_datasets = len(r.json())
        except: pass
        
        # Models
        try:
            r = requests.get(f"{BACKEND_URL}/api/models", timeout=1)
            if r.status_code == 200: n_models = len(r.json())
        except: pass
        
        # Gangs
        try:
            r = requests.get(f"{BACKEND_URL}/api/gangs", timeout=1)
            if r.status_code == 200: n_gangs = len(r.json())
        except: pass
    except:
        pass
