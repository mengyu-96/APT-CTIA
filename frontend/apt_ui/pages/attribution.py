...
def get_attribution_results():
    try:
        response = requests.get(f"{BACKEND_URL}/api/attribution_results", timeout=5)
        if response.status_code == 200:
            return response.json()
    except:
        pass
...
def get_attribution_detail(result_id):
    try:
        response = requests.get(f"{BACKEND_URL}/api/attribution_results/{result_id}", timeout=5)
        if response.status_code == 200:
            return response.json()
    except:
        pass
...
def get_server_tasks():
    ...
    try:
        response = requests.get(f"{BACKEND_URL}/api/tasks", timeout=2)
        if response.status_code == 200:
            tasks = response.json()
            # Filter for inference tasks and sort by creation time (newest first)
            inference_tasks = [t for t in tasks if t.get('type') == 'inference']
            ...
            with c_detail:
                if selected_result_id:
                    ...
                    if detail:
                        ...
                        # Pie Chart
                        if dist:
                            df_dist = pd.DataFrame(list(dist.items()), columns=['APT Group', 'Count'])
                        ...
                        if raw_results:
                            ...
                            for res in raw_results:
                                top3_str = ""
                                if res.get('top3'):
                                    # Safe access to label and score to avoid KeyErrors
                            ...
                            df_res = pd.DataFrame(formatted_results)
