...
import os
from apt_ui.services.api_client import clear_api_cache, get_artifact_bytes, get_json
...
def list_training_tasks():
    tasks = get_json("/api/tasks", timeout=5, default=[], ttl="fast")
...
@st.cache_data(ttl=300, show_spinner=False)
def _classification_report_df(report):
    ...
    with col_result:
        ...
        if 'current_model_result' in st.session_state:
            ...
            if result.get('history'):
                ...
                df_loss, df_acc = _history_frames(history)
            ...
            if result.get('classification_report'):
                with st.expander("详细分类报告 (Classification Report)"):
                     df_rep = _classification_report_df(result['classification_report'])
