...
def get_gangs_data():
    try:
        response = requests.get(f"{BACKEND_URL}/api/gangs", timeout=3)
        if response.status_code == 200:
            return response.json()
    except Exception as e:
        # Fallback to empty list or cache if backend is down, but prefer empty to avoid 'pseudo' state
        st.error(f"无法获取团伙数据: {e}")
    return []
